export const IDB_NAME = "fitly";
export const IDB_VERSION = 1;
export const IDB_READ = 'readonly';
export const IDB_READ_WRITE = 'readwrite';

export const IDB_TBL_STATS = "statistics";
export const IDB_TBL_BADGES = "badges";
export const IDB_TBL_PROGRESS = "progress";
export const IDB_TBL_DASHBOARD = "dashboard";
export const IDB_TBL_BODY_MEASUREMENT = "body_measurement";
export const IDB_TBL_BODY_LOGDATES = "body_logDates";
export const IDB_TBL_BODY_FAT = "body_fat";
export const IDB_TBL_BODY_PP = "body_progress_photos";
export const IDB_TBL_CALENDER = "calender";
export const IDB_TBL_EXERCISE = "exercise";
export const IDB_TBL_EXERCISE_DATA = "exercise_data";
export const IDB_TBL_WORKOUT_DATA = "workout_data";
export const IDB_TBL_USER_PROGRAM = "user_program";
export const IDB_TBL_FITNESS = "fitness";
export const IDB_TBL_PROFILE = "profile";
export const IDB_TBL_PROGRESS_PHOTO = "progress_photo";
export const IDB_TBL_POFILE_SETTING = "profile_setting";
export const IDB_TBL_SETTING = "setting";
export const IDB_TBL_SEARCH_USER = "search_user";
 



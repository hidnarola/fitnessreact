...
class ActivityYoursCard extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    var createdBy =
      post.created_by && Object.keys(post.created_by).length > 0
        ? post.created_by
        : null;
    if (!createdBy) {
      return null;
    ...
    postCreatedAt = moment(postCreatedAt)
      .local()
      .format("Do MMM [at] hh:mm");
    var type = post.type;
    var description = "";
    var images = [];
    ...
    var str = new String(description);
    description = str.replace(/(^|)(#.[a-z\d-]+)/gi, "$1<b>$2</b>");
    var newStr = description;
    ...
    if (totalComments > 0) {
      ...
      lastCommentCreatedAt = moment(lastCommentCreatedAt)
        .local()
        .format("Do MMM [at] hh:mm");
    }
    ...
    if (totalLikes > 0) {
      var likedOfLoggedUser = _.find(likes, [
        "authUserId",
    ...
    return (
      <div className="post-type timeline-infinite-scroll" key={post._id}>
        <div className="posttype-head d-flex justify-content-start">
          ...
          <h4 className="head_post_f">
            ...
            <p>{post.tag_line ? post.tag_line : ""}</p>
            <p className="">
              {postCreatedAt}
          ...
          <div className="posttype-btm d-flex">
            ...
            <a
              ...
              className="icon-chat"
              onClick={() => this.commentBoxRef.current.focus()}
            />
          </div>
          {/* {profile.friendshipStatus === FRIENDSHIP_STATUS_SELF &&
        ...
        <div className="posttype-body">
          {description &&
            description !== "null" && (
              <div className="activity-description m-1">
              ...
              images.map((imageD, imageI) => {
                ...
                return (
                  <div className="item" key={imageD._id}>
                    <a
                      href="javascript:void(0)"
                      onClick={() => handleOpenLightbox(images, imageI)}
                    >
                      <span key={imageI}>
                        <img
          ...
          {likesStr && (
            <div className="display-likes-status d-flex align-items-center m-2">
              ...
              <i className="fa fa-thumbs-up m-1" />
              <div className={cns("posttype-body-grey p-0")}>
                <p className="p-0">
                  <a href="#">{likesStr}</a>
                </p>
              </div>
              <div className="ml-auto comments-status">
                <p className="m-1">
                  <Link
                    to={`${routeCodes.POST}/${createdBy.username}/${post._id}`}
          ...
          {/* <div className={cns("posttype-body-grey")}>
            <p>
              {likesStr && (
                <a
                  href="javascript:void(0)"
                  onClick={() => toggleShowLikesModal(post._id)}
                >
                  {likesStr}
                </a>
              )}
              {totalComments > 0 && (
                <Link
                  to={`${routeCodes.POST}/${match.params.username}/${post._id}`}
                  className="pull-right"
                >
        ...
        {totalComments > 0 && (
          <div className="post-comment-box d-flex flex-wrap align-items-center">
            <span className="display-user-img">
              <img
            ...
            {totalComments > 0 && (
              <div className="display-comment-box">
                <div className="d-flex flex-wrap">
                  <p className="m-0">
                    <Link to={`${routeCodes.PROFILE}/${lastComment.username}`}>
                      {lastComment.firstName}{" "}
                      {lastComment.lastName ? lastComment.lastName : ""}
                    </Link>
                    <span
                      dangerouslySetInnerHTML={{
                        __html: replaceStringWithEmos(lastComment.comment)
                      }}
                    />
                  </p>
                </div>
                <div className="display-date">{lastCommentCreatedAt}</div>
              </div>
            )}
          </div>
        )}
...
ActivityYoursCard = connect(mapStateToProps)(ActivityYoursCard);

export default withRouter(ActivityYoursCard);

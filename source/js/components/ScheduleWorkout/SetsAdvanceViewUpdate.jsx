...
class SetsAdvanceViewUpdate extends Component {
    constructor(props) {
        super(props);
        this.state = {
            validations: []
    ...
    componentWillMount() {
        const {
            ...
            overrideDbValues,
            handleOverrideDbValues,
        } = this.props;
        if (selectedWorkoutForEdit.subType === SCHEDULED_WORKOUT_TYPE_EXERCISE) {
            if (overrideDbValues) {
                fields.removeAll();
                ...
                handleOverrideDbValues(true);
            }
        } else if (selectedWorkoutForEdit.subType === SCHEDULED_WORKOUT_TYPE_SUPERSET) {
            if (overrideDbValues) {
                fields.removeAll();
        ...
        } else if (selectedWorkoutForEdit.subType === SCHEDULED_WORKOUT_TYPE_CIRCUIT) {
            if (overrideDbValues) {
                fields.removeAll();
    ...
    componentDidUpdate(prevProps, prevState) {
        const { selectedWorkoutForEdit, superset_sets, circuit_sets, fields, exerciseMeasurements, selectedSingleExerciseObj } = this.props;
        const { validations } = this.state;
        let newValidations = [];
        fields.map((field, index) => {
...
export default connect(mapStateToProps)(SetsAdvanceViewUpdate);
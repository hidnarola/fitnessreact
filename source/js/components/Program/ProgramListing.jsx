...
class ProgramListing extends Component {
            ...
            <div className="white-box space-btm-20">
                    ...
                    <thead>
                      <tr>
                        ...
                        >
                          ...
                          {filterData &&
                            filterData.sort &&
                            typeof filterData.sort.name !== "undefined" &&
                            filterData.sort.name === 1 && (
                        ...
                        <th
                          className="cursor-pointer"
                          onClick={() => this.handleSort("totalWorkouts")}
                        >
                          <span>Workouts</span>
                          {filterData &&
                            filterData.sort &&
                            typeof filterData.sort.totalWorkouts !==
                              "undefined" &&
                            filterData.sort.totalWorkouts === 1 && (
                              <i className="icon-arrow_upward" />
                            )}
                          {filterData &&
                            filterData.sort &&
                        ...
                        >
                          ...
                          {filterData &&
                            filterData.sort &&
                            typeof filterData.sort.type !== "undefined" &&
                            filterData.sort.type === 1 && (
                              <i className="icon-arrow_upward" />
                            )}
                          {filterData &&
                            filterData.sort &&
                        ...
                        >
                          ...
                          {filterData &&
                            filterData.sort &&
                            typeof filterData.sort.difficulty !== "undefined" &&
                            filterData.sort.difficulty === 1 && (
                              <i className="icon-arrow_upward" />
                            )}
                          {filterData &&
                            filterData.sort &&
                        ...
                        {showRatingInList && (
                          ...
                          >
                            ...
                            {filterData &&
                              filterData.sort &&
                              typeof filterData.sort.rating !== "undefined" &&
                              filterData.sort.rating === 1 && (
                                <i className="icon-arrow_upward" />
                              )}
                            {filterData &&
                              filterData.sort &&
                    ...
                    <tbody>
                      {!loading &&
                        ...
                        programs.map((program, index) => {
                          let selectedGoalOption = _.find(SECONDARY_GOALS, [
                            "value",
                            program.goal
                          ]);
                          let selectedLevelOption = _.find(
                            PROGRAM_DIFFICULTY_LEVEL_OBJ,
                            ["value", program.level]
                          );
                          let goalLabel = selectedGoalOption
                            ? selectedGoalOption.label
                            : "-----";
                          let levelLabel = selectedLevelOption
                            ? selectedLevelOption.label
                            : "-----";
                          let frequencyLabel = "";
                          frequencyLabel += program.minWorkoutsCount
                            ? program.minWorkoutsCount
                            : 0;
                          if (
                            program.maxWorkoutsCount &&
                            program.maxWorkoutsCount > program.minWorkoutsCount
                          ) {
                          ...
                          return (
                            <tr key={index}>
                              ...
                              <td>
                                <span>{program.totalWorkouts}</span>
                              </td>
                              <td>
                              ...
                              {showRatingInList && (
                                <td>
                                  <span className="prog-rating-span-warp">
                                    <Link
                                      to={`${routeCodes.PROGRAMS_RATING_VIEW}/${
                                        program._id
                                      }`}
                                    >
                                      <RatingStarsDisplay
                                    ...
                                    <span>
                                      {program.programsRatingCount
                                        ? `${unitize(
                                            program.programsRatingCount
                                          )
                                            .capitalize()
                                            .precision(0)
                                            .toString(false)} reviews`
                                        : ""}
                                    </span>
                                  </span>
                                </td>
                                          ...
                                          <MenuItem
                                            ...
                                            onClick={e =>
                                              this.handleNavigation(
                                                e,
                                                `${routeCodes.PROGRAM_SAVE}/${
                                                  program._id
                                                }`
                                              )
                                            }
                                          >
                                            <FaPencil className="v-align-sub" />{" "}
                                            Edit
                                          </MenuItem>
                                          <MenuItem
                                            eventKey="2"
                                    ...
                                    program.userId !==
                                      ...
                                      <ButtonToolbar>
                                        <DropdownButton
                                          title="Actions"
                                          pullRight
                                          id="public-program-actions"
                                        >
                                          <MenuItem
                                            href={`${routeCodes.PROGRAM_VIEW}/${
                                              program._id
                                            }`}
                                            eventKey="1"
                                            onClick={e =>
                                              this.handleNavigation(
                                                e,
                                                `${routeCodes.PROGRAM_VIEW}/${
                                                  program._id
                                                }`
                                              )
                                            }
                                          >
                                            <FaEye className="v-align-sub" />{" "}
                                            View
                                          </MenuItem>
                                        </DropdownButton>
                                      </ButtonToolbar>
                      ...
                      {!loading &&
                        !(programs && programs.length > 0) && (
                          <tr>
                            <td colSpan={showRatingInList ? "7" : "6"}>
              ...
              <Pager className="program-pager-wrap">
                ...
                <Pager.Item
                  ...
                  disabled={
                    filterData.startFrom + (filterData.noOfRecords - 1) >=
                    totalRecords
                  }
                >
                  Next
  ...
  storeProgramInIDB = () => {
    ...
    try {
      const idbData = {
        type: MY_PROGRAMS,
        data: { programs: programs, totalRecords: totalRecords }
      };
...
export default connect(mapStateToProps)(ProgramListing);

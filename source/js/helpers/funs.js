...
export function prepareDropdownOptionsData(data, valueKey, labelKey) {
  let ddData = _.mapValues(data, o => {
    return { value: o[valueKey], label: o[labelKey] };
  });
...
export function ti(msg = "Something went wrong!") {
  toast.info(
    <span>
      <FaExclamationTriangle /> {msg}
    </span>
...
export function isSidebarOpen() {
  if (
    document.getElementById("manu-navigation").classList.contains("menu-hide")
  ) {
    console.log("false");
    return false;
  } else {
...
export function slideToBottomOfChatWindow(channelId) {
  ...
  var lastChildInnerHeight = elem
    .children()
    .last()
    .innerHeight();
  lastChildInnerHeight += 16; // as inner div contains 16px margin;
  if (
    elem.scrollTop() + elem.innerHeight() + lastChildInnerHeight >=
    elem[0].scrollHeight
  ) {
...
export function createNewStateForWorkout(workouts) {
  ...
  if (workouts && workouts.warmup && workouts.warmup.length > 0) {
    ...
    newState.workoutWarmupSequence =
      typeof workouts.warmup[lastIndex].sequence !== "undefined"
        ? workouts.warmup[lastIndex].sequence
        : -1;
  } else {
  ...
  if (workouts && workouts.exercise && workouts.exercise.length > 0) {
    ...
    newState.workoutSequence =
      typeof workouts.exercise[lastIndex].sequence !== "undefined"
        ? workouts.exercise[lastIndex].sequence
        : -1;
  } else {
  ...
  if (workouts && workouts.cooldown && workouts.cooldown.length > 0) {
    ...
    newState.workoutCooldownSequence =
      typeof workouts.cooldown[lastIndex].sequence !== "undefined"
        ? workouts.cooldown[lastIndex].sequence
        : -1;
  } else {
...
export function calculateBodyFatPercentage(sumOf3Sites, age, gender) {
  ...
  return (
    const1 /
      (const2 -
        const3 * sumOf3Sites +
        const4 * sumOf3Sites * sumOf3Sites -
        const5 * age) -
    const6
  ).toFixed(2);
}
  ...
  if (str) {
    ...
    return str.replace(REGEX_FOR_EMOJI_COLONS, match => {
      return ReactDOMServer.renderToString(
        <Emoji emoji={match} set="emojione" size={size} />
      );
    });
  }
  return str;

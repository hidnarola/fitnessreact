  ...
  return function*(action) {
    try {
      ...
      const data = yield call(() =>
        api.deleteUsersProgramWorkoutSchedule(requestData)
      );
...
function deleteUserProgramBulkExerciseData() {
  return function*(action) {
    try {
      let requestData = action.requestData;
      const data = yield call(() =>
        api.deleteUserProgramBulkExercise(requestData)
      );
...
function addUserProgramWorkoutTitleData() {
  return function*(action) {
    try {
      let requestData = action.requestData;
      const data = yield call(() =>
        api.addUserProgramWorkoutTitle(requestData)
      );
...
function viewUsersPublicProgramWorkoutScheduleData() {
  return function*(action) {
    try {
      let _id = action._id;
      const data = yield call(() =>
        api.viewUsersPublicProgramWorkoutSchedule(_id)
      );
...
function addUsersProgramWorkoutScheduleData() {
  return function*(action) {
    try {
      let requestData = action.requestData;
      const data = yield call(() =>
        api.addUsersProgramWorkoutSchedule(requestData)
      );
...
function updateUserProgramWorkoutTitleData() {
  return function*(action) {
    try {
      ...
      let requestData = action.requestData;
      const data = yield call(() =>
        api.updateUserProgramWorkoutTitle(_id, requestData)
      );
...
function updateUsersProgramWorkoutScheduleData() {
  return function*(action) {
    try {
      let requestData = action.requestData;
      const data = yield call(() =>
        api.updateUsersProgramWorkoutSchedule(requestData)
      );
...
function deleteUserProgramSingleExerciseData() {
  return function*(action) {
    try {
      let requestData = action.requestData;
      const data = yield call(() =>
        api.deleteUserProgramSingleExercise(requestData)
      );
...
function deleteUserProgramWholeExerciseData() {
  return function*(action) {
    try {
      let requestData = action.requestData;
      const data = yield call(() =>
        api.deleteUserProgramWholeExercise(requestData)
      );
...
function pasteUsersProgramWorkoutScheduleData() {
  return function*(action) {
    try {
      ...
      const data = yield call(() =>
        api.pasteUsersProgramWorkoutSchedule(requestData, _action)
      );
...
function updateUserProgramMasterData() {
  return function*(action) {
    try {
      ...
      let requestData = action.requestData;
      const data = yield call(() =>
        api.updateUsersProgramMaster(_id, requestData)
      );
...
function getWorkoutsListByProgramDay() {
  return function*(action) {
    try {
      let requestData = action.requestData;
      const data = yield call(() =>
        api.getWorkoutsListByProgramDay(requestData)
      );
...
function reorderProgramWorkoutExercises() {
  return function*(action) {
    try {
      let requestData = action.requestData;
      const data = yield call(() =>
        api.reorderProgramWorkoutExercises(requestData)
      );
...
function createUserProgramFromCalendarData() {
  return function*(action) {
    try {
      ...
      const data = yield call(() =>
        api.createUserProgramFromCalendar(requestData)
      );
...
function appendUserProgramFromCalendarData() {
  return function*(action) {
    try {
      const requestData = action.requestData;
      const data = yield call(() =>
        api.appendUserProgramFromCalendar(requestData)
      );
...
export function* watchUserProgramsData() {
  ...
  yield takeLatest(
    VIEW_USER_PUBLIC_PROGRAM_REQUEST,
    viewUserPublicProgramData()
  );
  yield takeLatest(GET_USER_PROGRAM_MASTER_REQUEST, getUserProgramMasterData());
  yield takeLatest(
    DELETE_USERS_PROGRAM_WORKOUT_SCHEDULE_REQUEST,
    deleteUsersProgramWorkoutScheduleData()
  );
  yield takeLatest(
    ADD_USER_PROGRAM_WORKOUT_TITLE_REQUEST,
    addUserProgramWorkoutTitleData()
  );
  yield takeLatest(
    GET_USERS_PROGRAM_WORKOUT_SCHEDULE_REQUEST,
    getUsersProgramWorkoutScheduleData()
  );
  yield takeLatest(
    VIEW_USERS_PUBLIC_PROGRAM_WORKOUT_SCHEDULE_REQUEST,
    viewUsersPublicProgramWorkoutScheduleData()
  );
  yield takeLatest(
    ADD_USERS_PROGRAM_WORKOUT_SCHEDULE_REQUEST,
    addUsersProgramWorkoutScheduleData()
  );
  yield takeLatest(
    UPDATE_USER_PROGRAM_WORKOUT_TITLE_REQUEST,
    updateUserProgramWorkoutTitleData()
  );
  yield takeLatest(
    UPDATE_USERS_PROGRAM_WORKOUT_SCHEDULE_REQUEST,
    updateUsersProgramWorkoutScheduleData()
  );
  yield takeLatest(
    DELETE_USER_PROGRAM_SINGLE_EXERCISE_REQUEST,
    deleteUserProgramSingleExerciseData()
  );
  yield takeLatest(
    DELETE_USER_PROGRAM_WHOLE_EXERCISE_REQUEST,
    deleteUserProgramWholeExerciseData()
  );
  yield takeLatest(
    DELETE_USER_PROGRAM_BULK_EXERCISE_REQUEST,
    deleteUserProgramBulkExerciseData()
  );
  yield takeLatest(
    PASTE_USERS_PROGRAM_WORKOUT_SCHEDULE_REQUEST,
    pasteUsersProgramWorkoutScheduleData()
  );
  yield takeLatest(
    UPDATE_USER_PROGRAM_MASTER_REQUEST,
    updateUserProgramMasterData()
  );
  yield takeLatest(
    GET_WORKOUTS_LIST_BY_PROGRAM_DAY_REQUEST,
  ...
  yield takeLatest(
    CREATE_USER_PROGRAM_FROM_CALENDAR_REQUEST,
    createUserProgramFromCalendarData()
  );
  yield takeLatest(
    APPEND_USER_PROGRAM_FROM_CALENDAR_REQUEST,
    appendUserProgramFromCalendarData()
  );
}

export default [watchUserProgramsData()];

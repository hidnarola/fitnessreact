...
import {
  ...
  getUserFitnessTestsLogDatesSuccess,
  getUserFitnessTestsLogDatesError,
...
function getUserFitnessTestsLogDatesData() {
  return function*(action) {
    try {
      var today = action.today;
      const data = yield call(() => api.getUserFitnessTestsLogDates(today));
      yield put(getUserFitnessTestsLogDatesSuccess(data));
    } catch (error) {
      yield put(getUserFitnessTestsLogDatesError(error));
    }
  };
}
...
export function* watchUserFitnessTestsData() {
  ...
  yield takeLatest(
    GET_USER_FITNESS_TESTS_LOGDATES_REQUEST,
    getUserFitnessTestsLogDatesData(),
  );
  yield takeLatest(SAVE_USER_FITNESS_TESTS_REQUEST, saveUserFitnessTestsData());
  yield takeLatest(
    RESET_USER_FITNESS_TESTS_REQUEST,
    resetUserFitnessTestsData(),
  );
}

export default [watchUserFitnessTestsData()];

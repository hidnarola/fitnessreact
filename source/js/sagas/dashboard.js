...
import {
  ...
  followingUserActivityDataSuccess,
  followingUserActivityDataError,
  ...
  return function*(action) {
    try {
      ...
      const data = yield call(() =>
        api.changeDashboardBodyFatWidget(requestData)
      );
...
function changeDashboardMuscleInnerData() {
  return function*(action) {
    try {
      let requestData = action.requestData;
      const data = yield call(() =>
        api.changeDashboardMuscleInnerData(requestData)
      );
...
function followingUserActivityData() {
  return function*(action) {
    try {
      const data = yield call(() =>
        api.followingUserActivityData(action.start, action.offset)
      );
      yield put(followingUserActivityDataSuccess(data));
    } catch (error) {
      yield put(followingUserActivityDataError(error));
    }
  };
}
...
export function* watchDashboardData() {
  ...
  yield takeLatest(
    CHANGE_DASHBOARD_BODY_FAT_WIDGET_REQUEST,
    changeDashboardBodyFatWidgetData()
  );
  yield takeLatest(
    CHANGE_COMPLETE_STATUS_OF_WORKOUT_REQUEST,
    changeCompleteStatusOfWorkoutData()
  );
  yield takeLatest(
    CHANGE_DASHBOARD_MUSCLE_INNER_DATA_REQUEST,
...
export default [watchDashboardData()];
